"""
Post-processing for raw thermal model detections.

This module is the only place in the thermal detection pipeline that:

* filters by confidence threshold
* filters by allowed class names
* clamps bounding boxes to frame boundaries
* validates bounding-box geometry (x1 < x2, y1 < y2)
* converts raw model output into :class:`~.models.ThermalDetection`
...
class ThermalPostProcessor:
    """Converts and filters raw model boxes into :class:`ThermalDetection` objects.
    ...
    allowed_classes:
        Set of class name strings to keep.  ``None`` accepts all classes.

    Notes
    -----
    This class holds no per-frame state; a single instance can safely
    ...
    def process(
        ...
        """Convert raw model arrays into a filtered list of :class:`ThermalDetection`.
        ...
        for box, conf, cls_id in zip(boxes_xyxy, confidences, class_ids):
            ...
            if conf_float < self._conf_threshold:
                logger.debug(
                    "Skipping detection: confidence %.3f < %.3f",
                    conf_float,
            ...
            if cls_int < 0 or cls_int >= len(class_names):
                logger.warning(
                    "Class id %d out of range for class list (len=%d); "
                    "skipping.",
                    cls_int,
                    len(class_names),
                )
            ...
            if (
                ...
                and cls_name not in self._allowed_classes
            ):
                logger.debug("Skipping disallowed class: %r", cls_name)
                continue
        ...
        logger.debug(
            "ThermalPostProcessor produced %d detection(s).",
            len(detections),
        )
        return detections
    ...
    def _clamp_and_validate_bbox(
        ...
        -------
        tuple or None
            Clamped ``(x1, y1, x2, y2)`` floats, or ``None`` if the box
            is degenerate after clamping.
        """
        x1 = float(box[0])
        y1 = float(box[1])
        x2 = float(box[2])
        ...
        if x1 >= x2 or y1 >= y2:
            logger.warning(
                "Degenerate bbox after clamping: "
                "(%.1f, %.1f, %.1f, %.1f); skipping.",
                x1,
                y1,
                x2,
                y2,
